...
clustering <- function(inst, k, L, eps = 0, variances, info, cluster_method = c("greedy", "local_search"), alpha = 1) {
  ...
  # inst = test_instances$p8_christofides; k = 3; L = 100; eps = 0; cluster_method = "local_search"; variances = generate_variances(inst); alpha = 1; info <- generate_information(inst, r = 20)
  inst$info <- info
  ...
  in_points <- inst$points |> dplyr::filter(point_type == "node", in_range)
  ...
  local_search_clustering <- function() {
    ...
    cluster_eval2 <- function(zone) {
      ...
      tibble::tibble("profit" = total_profit, "avg_dist" = avg_dist)
    }
...
# clust <- clustering(inst = test_instances$p8_christofides, k = 3, L, variance = generate_variances(inst = test_instances$p8_christofides), info = generate_information(inst, r = 20), cluster_method = "local_search")
...
plot.clustering <- function(clust, delaunay = T) {
  ...
  p +
    ...
    ggplot2::geom_point(
      data = clust$instance$points |> dplyr::filter(point_type %in% c("source", "sink")),
