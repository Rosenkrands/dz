...
most_improvement <- (combined_results |>
  mutate(max_ur_score = sapply(ur_score, max)) |>
  arrange(-(max_ur_score - sr_score)))[7,]

zones <- most_improvement$clust[[1]]$cl$zones
k <- most_improvement$k[1]
L <- most_improvement$L[1]/most_improvement$k[1]
